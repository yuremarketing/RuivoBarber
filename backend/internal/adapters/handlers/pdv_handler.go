...
func (h *PdvHandler) AbrirCaixa(c *fiber.Ctx) error {
	...
	caixa, err := h.service.AbrirCaixa(c.UserContext(), operadorID, req.SaldoInicial)
	if err != nil {
...
func (h *PdvHandler) FecharCaixa(c *fiber.Ctx) error {
	...
	operadorID := c.Locals("userId").(int)
	caixa, err := h.service.FecharCaixa(c.UserContext(), operadorID, req.SaldoInformado)
	if err != nil {
		return c.Status(400).JSON(fiber.Map{"error": err.Error()})
	}
...
func (h *PdvHandler) ObterStatusCaixa(c *fiber.Ctx) error {
	...
	operadorID := c.Locals("userId").(int)
	status, err := h.service.ObterStatusCaixa(c.UserContext(), operadorID)
	if err != nil {
...
func (h *PdvHandler) MovimentarCaixa(c *fiber.Ctx) error {
	...
	operadorID := c.Locals("userId").(int)
	err := h.service.MovimentarCaixa(c.UserContext(), operadorID, req.Tipo, req.Valor, req.Motivo)
	if err != nil {
		return c.Status(400).JSON(fiber.Map{"error": err.Error()})
	}
...
func (h *PdvHandler) ProcessarVenda(c *fiber.Ctx) error {
	...
	operadorID := c.Locals("userId").(int)
	venda, err := h.service.ProcessarVenda(c.UserContext(), operadorID, &req)
	if err != nil {
		return c.Status(400).JSON(fiber.Map{"error": err.Error()})
	}
