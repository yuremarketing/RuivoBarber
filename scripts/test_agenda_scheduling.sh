...
echo "Consultando slots iniciais para o barbeiro $BARB_ID em 2026-06-20..."
SLOTS_RESP=$(curl -s -H "Authorization: Bearer $TOKEN" "$API_URL/api/v1/barbeiros/$BARB_ID/agenda?data=2026-06-20&servico_id=2")
...
CREATE_RESP=$(curl -s -w "\n%{http_code}" -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d "{\"barbeiro_id\": $BARB_ID, \"servico_id\": 2, \"data_hora\": \"2026-06-20 09:30\"}" "$API_URL/api/v1/agendamentos")
...
SLOTS_RESP2=$(curl -s -H "Authorization: Bearer $TOKEN" "$API_URL/api/v1/barbeiros/$BARB_ID/agenda?data=2026-06-20&servico_id=2")
...
CONFLICT_RESP=$(curl -s -w "\n%{http_code}" -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d "{\"barbeiro_id\": $BARB_ID, \"servico_id\": 1, \"data_hora\": \"2026-06-20 10:00\"}" "$API_URL/api/v1/agendamentos")
